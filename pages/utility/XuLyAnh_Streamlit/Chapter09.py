...
def ConnectedComponent(imgin):
    ret, temp = cv2.threshold(imgin, 200, 255, cv2.THRESH_BINARY)
    temp = cv2.medianBlur(temp, 7)

    # Đảm bảo rằng temp là một ảnh grayscale với kiểu dữ liệu là np.uint8
    ...
    # Sử dụng int thay vì np.int
    a = np.zeros(dem, int)
    M, N = label.shape
    color = 150

    # Tạo một bản sao của label và chuyển đổi kiểu dữ liệu thành np.uint8
    label_copy = np.copy(label).astype(np.uint8)
    ...
    for x in range(0, M):
        for y in range(0, N):
            ...
            a[r] = a[r] + 1
            if r > 0:
                label_copy[x, y] = label_copy[x, y] + color

    for r in range(1, dem):
        print('%4d %10d' % (r, a[r]))

    cv2.putText(label_copy, text, (1, 25), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
    return label_copy
...
def CountRice(imgin):
    ...
    ret, temp = cv2.threshold(gray_temp, 100, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
